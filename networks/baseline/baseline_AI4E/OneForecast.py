...
def add_selected_pairs_for_latlon(
    ...
    vertex_latlon = xyz2latlon(vertices_xyz, unit="deg")  # [N, 2], in degrees
    if unit == "rad":
        factor = 180.0 / np.pi
...
if __name__ == '__main__':
    ...
    net = OneForecast(input_res=(360, 720), input_dim_grid_nodes=59, output_dim_grid_nodes=53).to(device)

    input = torch.randn(1, 59, 360, 720).to(device)
    output = net(input)
